...
IR_sit <- .5 * IR("sitting", "male") + .5 * IR("sitting", "female")
WR_vol <- convert_dist(cellSize)^2 * dimWR[3]
round((1 - exp(-max(main_qc_wr_mo$value) * 600 * IR_sit / WR_vol)) * 100, 0)
round((1 - exp(-max(main_qc_wr_af$value) * 600 * IR_sit / WR_vol)) * 100, 0)
...
mean_quanta_pl <- plot_spatial(
  ggplot(mapping = aes(x = long, y = lat, group = group, fill = value)) +
    ...
    labs(
      fill = expression("Mean quanta concentration (quanta/m"^3 * " x 10e"^-3 * " per sec)"),
      title = "a"
    ) +
    scale_fill_gradientn(
      colours = RColorBrewer::brewer.pal(n = 9, name = "YlOrRd"),
...
amod_est %>%
  ...
  mutate(across(c(lower, median, upper), ~ round_k(.x, 2)))
