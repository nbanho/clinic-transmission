...
(3)~We calculated \added{time-varying} room occupancy from the video sensor data and combined it with the CO$_2$ measurements \added{(Fig.~\zref{fig:co2-levels}-\zref{fig:people-over-time-by-day} in \supp)} to the estimate air change rate during the morning (8\,am to 12\,am) and afternoon (12\,am to 4\,pm) of each clinic day using a transient mass balance model\cite{Batterman2017IJERPH}. 
...
(5)~Quanta removal occurs through the replacement of contaminated indoor air with outdoor air, inactivation of bacteria in airborne particles, and gravitational settling. The removal rate is thus the sum of the air change rate, the inactivation rate of \emph{Mtb}, and the gravitational settling rate. We modeled the bacterial inactivation rate of \emph{Mtb} using a lognormal prior distribution (Fig.~\zref{fig:undiagnosed-distribution} in \supp) with a median of 1\,\deleted{quanta }h$^{-1}$ (95\%-CrI 0.1\,\deleted{quanta }h$^{-1}$\,$-$\,7.1\,\deleted{quanta }h$^{-1}$)\cite{Loudon1969AMRRD,Lever2000LettersAppliedMicrobio,Gannon2007ResVetSci,Klein2014IJMyco}. \deleted{We set the gravitational settling rate to zero and ignore sedimentation because} Particles with \emph{Mtb} in the size range of $1-7\mu$m\cite{Fennelly2020Lancet} have a settling velocity of \deleted{just} $3.5\cdot 10^{-5}-1.5\cdot 10^{-3}$m\,s$^{-1}$ in still air\cite{Vuorinen2020SafSci}, \added{which we modeled using a Gamma distribution with mean $7.7\cdot 10^{-3}$m\,s$^{-1}$ and standard deviation $3.7\cdot 10^{-3}$m\,s$^{-1}$. Considering a drop height of 1.7\,m, the median gravitational settling rate was assumed to be 1.5 h$^{-1}$ (95\%-CrI 0.5 h$^{-1}$ – 3.5 h$^{-1}$).}

\subsubsection*{Risk of infection}

(6)~\deleted{Based on the quanta generation, diffusion, and removal process, the quanta concentration $N$ (quanta\,m$^{-3}$) at time $t$ was computed as}\added{The quanta concentration was computed separately for the waiting room, corridor, and TB room. Each room was discretized into a grid of cubic cells, with each cell covering a squared area of 0.0625\,m$^2$. We assumed that the quanta concentration was vertically well-mixed and computed the quanta concentration $N_{r,c,t}$ in room $r$ and cell $c$ at time $t$ as} 
\begin{align}\label{eq:spattemp-N}
    % \deleted{\underbrace{N_{t}}_{\text{new conc.}} = \underbrace{\left(D \Delta (\underbrace{N_{t-1}}_{\text{prev. conc.}} + \underbrace{I_t \cdot q \cdot 1\text{s}}_{\text{generation}}) \cdot 1\text{s}\right)}_{\text{diffusion}} \cdot \underbrace{\exp\left(-(AER_t + \lambda\cdot 1\text{s}\right)}_{\text{removal}} ~.}
     \added{\underbrace{N_{r,c,t}}_{\text{new conc.}} = \underbrace{\left(D \Delta (\underbrace{N_{r,c,t-1}}_{\text{old conc.}} + \underbrace{I_{r,c,t} \cdot q\cdot1\,s}_{\text{generation}}\cdot V_{r,c}^{-1})\right)}_{\text{diffusion}} \cdot \underbrace{\exp\left(-(AER_{r,t} + \lambda + k)\cdot1\,s\right)}_{\text{removal}} ~.}
\end{align}
where \deleted{$D$}\added{$D_t$} is the diffusion constant (m$^2$\,s$^{-1}$) \added{varying by daytime}, $\Delta$ is the Laplace operator (second-order differential operator), \deleted{$I_t$ is the number of infectious individuals in space, } \added{$I_{r,c,t}$ is the number of infectious individuals, $V_{r,c}$ is the room-specific cell volume (m$^3$)}, $q$ is the quantum generation rate (quanta\,s$^{-1}$), \deleted{$AER$ is the}\added{$AER_{r,t}$ is the room-specific} air change rate \added{varying by daytime}(\deleted{quanta}s$^{-1}$), $\lambda$ is the bacterial inactivation rate (\deleted{quanta}s$^{-1}$), \added{and $k$ is the gravitational settling rate (s$^{-1}$)}. \added{We modeled the spatiotemporal quanta concentration during clinic hours from 8\,am to 4\,pm and updated it every second, corresponding to the frequency of the person-tracking data.} 
...
\begin{align}
    ...
    \added{P = 1-\exp\left(-\sum_r \sum_c \sum_t N_{r,c,t} \cdot \mathbb{I}_{r,c,t} \cdot p_a\cdot1\,s\right),}
\end{align}
where \deleted{$N_{c,t}$ is the quanta concentration in cell space $c$ at time $t$, $\mathbb{I}_{c,t}$ indicates whether the person was in $c$ at $t$}\added{$\mathbb{I}_{r,c,t}$ indicates whether the person was in cell $c$ of room $r$ at time $t$}, and $p_a$ (m$^3$ s$^{-1}$) is the breathing rate for activity level $a$. We distinguished between walking ($p_\mathrm{walk}$ = 1.33\,m$^3$\,h$^{-1}$) and sitting activities ($p_\mathrm{sit}$ = 0.51\,m$^3$\,h$^{-1}$)\cite{Adams1993}, based on whether the the person had moved by more or less than 0.25\,m\,s$^{-1}$. \added{We assumed no effect of mask use on the inhalation rate of quanta\cite{McCreesh2021BMJGlobalHealth}.}

\subsection*{\deleted{Monte Carlo simulation}}

\deleted{Simulations are described in detail in Section~\zref{sec:estimation} in \supp. Briefly, we simulated the spatiotemporal quanta concentration separately for the clinic's waiting room, corridor, and TB room. Each room was rasterized into a grid of cubic cells with an area of 0.25$^2$\,m$^2$. The quanta concentration was modeled during clinic hours from 8\,am to 4\,pm and updated every second. We used a Monte-Carlo simulation approach to consider uncertainty in modeling parameters. In each simulation, we sampled uncertain modeling parameters from their prior distributions and computed the spatiotemporal quanta concentration and the risk of infection per clinic attendee.}
...
The quanta concentration was higher in the morning than in the afternoon, and higher in the waiting room than in the corridor or TB room of the clinic (\Cref{fig:main-modeling-results}). An exposure of one hour to the mean quanta concentration in the middle of the waiting room would translate to a \deleted{0.40\%}\added{0.20\%} risk of infection in the morning compared to a \deleted{0.07\%}\added{0.04\%} risk in the afternoon. The mean risk of infection per clinic visit was \deleted{0.1\% (80\%-credible interval (CrI) 0.01\%$-$0.1\%)}\added{0.05\% (80\%-credible interval (CrI) 0.01\%$-$0.06\%)}, corresponding to a risk of \deleted{0.21\% per hour (80\%-CrI 0.03\%$-$0.36\%)}\added{0.1\% per hour (80\%-CrI 0.01\%$-$0.21\%)}. S2~Video shows an animated sequence of the spatiotemporally varying quanta concentration as an infectious patient enters the clinic on the morning of October~25 and moves through the waiting room. It can be seen that the quanta concentration was higher near the infectious patient, following their position and movement. The quanta diffused quickly because the diffusion constant was related to the air change rate, which was generally high because all doors and windows were typically open during clinic hours.

The modeled individual risk of infection was associated with the number of close contacts and the time spent in the clinic. A doubling of close-contact encounters increased the odds of infection by \deleted{23\% (OR 1.23, 95\%-CrI 1.09$-$1.37)}\added{17\% (OR 1.17, 95\%-CrI 1.04$-$1.30)}. Doubling the time spent in the clinic increased the odds of infection by \deleted{12\% (OR 1.12, 95\%-CrI 1.03$-$1.21)}\added{12\% (OR 1.12, 95\%-CrI 1.03$-$1.22)}. The time spent in close contact was not significantly associated with the individual risk of infection \deleted{(OR 1.03, 95\%-CrI, 0.94$-$1.13)}\added{(OR 1.02, 95\%-CrI, 0.94$-$1.11)}. 
...
The hypothetical scenarios assuming no mask use and pre-pandemic ventilation rates are shown in  \Cref{fig:scenario-results}. If none of the clinic attendees or staff had worn masks, the mean risk of infection per visit would have been \deleted{0.12\% (80\%-CrI 0.02\%$-$0.13\%)}\added{0.07\% (80\%-CrI 0.01\%$-$0.08\%)}, an increase of 30\% (OR 1.3, 95\%-CrI 1.2$-$1.4). If the air change rate had been the same as before the COVID-19 pandemic, the mean risk of infection would have been \deleted{0.19\% (80\%-CrI 0.02\%$-$0.20\%)}\added{0.09\% (80\%-CrI 0.01\%$-$0.10\%)}, an increase of \deleted{120\% (OR 2.2, 95\%-CrI 0.9$-$5.2)}\added{110\% (OR 2.1, 95\%-CrI 0.9$-$5.0)}. Without mask use and with pre-pandemic ventilation rates, the mean risk of infection would have been \deleted{0.24\% (80\%-CrI 0.03\%$-$0.24\%)}\added{0.12\% (80\%-CrI 0.02\%$-$0.12\%)}, an increase of \deleted{180\% (OR 2.8, 95\%-CrI 1.1$-$6.6)}\added{170\% (OR 2.7, 95\%-CrI 1.1$-$6.6)}. The scenarios show an increase in the number of clinic attendees with a high risk of infection (\Cref{fig:scenario-results}). 

In sensitivity analyses, we varied assumptions about the number of infectious persons in the clinic. Assuming that the number of infectious TB patients reflected the prevalence of TB in the South African population, the mean risk of infection would have been slightly higher \deleted{(0.12\%, 80\%-CrI 0.03\%$-$0.19\%)}\added{(0.06\%, 80\%-CrI 0.01\%$-$0.10\%)}. Further, assuming that both diagnosed TB patients and clinic attendees suspected of having TB were infectious, the mean risk of infection would have been twice as high \deleted{(0.22\%, 80\%-CrI <0.01\%$-$0.43\%)}\added{(0.11\%, 80\%-CrI <0.01\%$-$0.18\%)}.  
...
% limitations
Our study has several limitations. First, tracking patient movements with video sensors was challenging. It required considerable data processing and many interrupted tracks could not be reconnected. Therefore, some clinic attendees may have had longer visits and the risk of infection may have been underestimated. Second, several important assumptions had to be made regarding the generation, diffusion, and removal of infectious quanta. \added{For example, the waiting room and corridor were separated to facilitate modeling and because we observed differences in room occupancy and time-varying CO$_2$ levels, yet the door between these areas was typically open, so that aerosolized \emph{Mtb} (quanta) could have spread from one area to the other.} We also assumed that infectious particles move radially from the source and approach a steady state with uniform contamination of the airspace, which may depend on airflow. However, airflow is very difficult to measure. Furthermore, uncertainty in modeling parameters determining quanta generation and removal was often large and could be reduced in future work by incorporating, for example, additional information on patient characteristics from clinical records that are known to be associated with infectiousness\cite{Escombe2008PLoSMed} or susceptibility\cite{Furin2019Lancet}. Third, \added{we assumed that mask use only had an effect on the exhalation of \emph{Mtb} particles. Although the same assumption was made in a related modeling study\cite{McCreesh2021BMJGlobalHealth}, we note that our modeled risk of infection would be lower if we assumed an additional effect of mask use on inhalation. Fourth,} we could not validate our modeling results with empirical findings. Future research could test the practical implementation of our model by examining whether clinic attendees with a high risk of infection were also subsequently more likely to be diagnosed with TB.  
