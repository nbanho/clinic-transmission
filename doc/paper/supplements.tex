...
An analysis of exhalation activities suggest a propagation distance (\ie the maximum distance from the source to the end of the exhaled air before dispersion) of 0.7m\cite{Tang2013PLoSOne}, but larger distances may be reached depending on the activity. Face masks reduce the propagation distance as most exhaled air (and particles) by locking the formation of the jet and leaking air on the sides of the masks\cite{Tang2009RoyalInt,Hui2012PLoSOne}. Therefore, we assume that mask wearing confined the quanta generation only to the cell where the infectious individual is located. This corresponds to a propagation distance of 0.35m (the diagonal cell length) and an area of 0.25$^2$m$^2$. 
...
\begin{figure}[!htpb]
    ...
    \caption{Prior distribution function for the viral inactivation rate along with reported estimates in the literature: 0.12 by Loudon (1969) [REF], 0.46 by Gannon (2007) [REF], 3.2 by Klein (2014) [REF], and 8.3 by Lever (2000) [REF].}
