...
match_clinical_tracking <- function(
  ...
  sub_titl <- paste(
    "Clinical records:", n_clinical_pid, "considered,", n_tot_infectious, "diagnosed,", n_tot_suspected, "suspected.\nLinked:",
  ...
  return(list(track_linked = tracking_linked_clinical, linked = merged))
}
...
for (i in 1:length(selected_dates)) {
  ...
  merged_report[[i]] <- matched_df$linked
  saveRDS(matched_df$track_linked, paste0(save_dir, selected_dates[i], ".rds"))
}

# merge report summary
merged_report <- do.call(rbind, merged_report)
round(merged_report$timediff / 60, 1)
summary(merged_report$timediff / 60, 1)
nrow(merged_report)
