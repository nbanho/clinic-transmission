...
parallelogram <- function(x, y, w, b, r, up = TRUE) {
  # vertical movement
  dx <- b / w * (x[2] - y[2])
  dy <- b / w * (y[1] - x[1])
...
line_point <- function(pt, l, r, start = TRUE) {
  dy <- sin(r * pi / 180) * l
  dx <- cos(r * pi / 180) * l
  if (start) {
...
outline <- parallelogram(
  bottom_left_pt, bottom_right_pt,
  width, breadth, rotation
)
...
wr <- parallelogram(
  wr_top_left_pt, wr_top_right_pt,
  wr_width, -wr_breadth, rotation
)
...
sp_bottom_left_pt <- line_point(
  sp_bottom_right_pt, sp_width, rotation,
  start = FALSE
)
sp <- parallelogram(
  sp_bottom_left_pt, sp_bottom_right_pt,
  sp_width, sp_breadth, rotation
)
...
tb <- parallelogram(
  tb_bottom_left_pt, tb_bottom_right_pt,
  tb_width, tb_breadth, rotation
)
...
pa <- parallelogram(
  pa_top_left_pt, pa_top_right_pt,
  pa_width, -pa_breadth, rotation
)

# reception
re_width <- 1500
re_breadth <- euclidean(wr[3, 1], pa[4, 1], wr[3, 2], pa[4, 2]) + 500
re_bottom_left_pt <- line_point(wr[3, ], re_width, rotation, start = FALSE)
re <- parallelogram(re_bottom_left_pt, wr[3, ], re_width, re_breadth, rotation)

# entrance
er_width <- 6500
er_breadth <- breadth - wr_breadth
er_top_left_pt <- line_point(wr[3, ], er_width, rotation, start = FALSE)
er <- parallelogram(
  er_top_left_pt, wr[3, ],
  er_width, -er_breadth, rotation
)
...
sa_wr <- parallelogram(
  wr[4, ], sa_wr_bottom_right_pt,
  sa_wr_width, sa_wr_breadth, rotation
)

# seating area passage left to TB room
sa_pa_leftTB_width <- 2375
sa_pa_leftTB_breadth <- 1500
sa_pa_leftTB_bottom_left_pt <- line_point(
  tb[4, ], sa_pa_leftTB_width, rotation,
  start = FALSE
)
sa_pa_leftTB <- parallelogram(
  tb[4, ], sa_pa_leftTB_bottom_left_pt,
  sa_pa_leftTB_width, -sa_pa_leftTB_breadth
)
...
sa_pa_rightTB <- parallelogram(
  sa_pa_rightTB_bottom_right_pt, tb[4, ],
  sa_pa_rightTB_width, -sa_pa_leftTB_breadth, rotation
)
...
er_re_top_left_pt <- line_point(
  er_re_top_right_pt, er_re_width, rotation,
  start = FALSE
)
er_re <- parallelogram(
  er_re_top_left_pt, er_re_top_right_pt,
  er_re_width, -er_re_breadth, rotation
)

# seating area passage right to reception
sa_pa_rightRE_width <- 2700
sa_pa_rightRE_bottom_left_pt <- pa[4, ]
sa_pa_rightRE_bottom_right_pt <- line_point(
  sa_pa_rightRE_bottom_left_pt, sa_pa_rightRE_width, rotation
)
sa_pa_rightRE <- parallelogram(
  sa_pa_rightRE_bottom_left_pt, sa_pa_rightRE_bottom_right_pt,
  sa_pa_rightRE_width, sa_pa_leftTB_breadth, rotation
)
...
er_cr_oppTBleft_bottom_right_pt <- line_point(
  er_cr_oppTBleft_bottom_left_pt, er_cr_oppTBleft_width, rotation
)
er_cr_oppTBleft <- parallelogram(
  er_cr_oppTBleft_bottom_left_pt, er_cr_oppTBleft_bottom_right_pt,
  er_cr_oppTBleft_width, er_cr_oppTBleft_breadth, rotation
)
...
er_cr_oppTBright_bottom_right_pt <- line_point(
  er_cr_oppTBright_bottom_left_pt, er_cr_oppTBright_width, rotation
)
er_cr_oppTBright <- parallelogram(
  er_cr_oppTBright_bottom_left_pt, er_cr_oppTBright_bottom_right_pt,
  er_cr_oppTBright_width, er_cr_oppTBright_breadth, rotation
)
...
exl <- parallelogram(
  exl_bottom_left_pt, exl_bottom_right_pt,
  exl_width, exl_breadth, rotation
)
...
exr <- parallelogram(
  exr_bottom_left_pt, exr_bottom_right_pt,
  exr_width, exr_breadth, rotation
)

# combine to form clinic
clinic <- rbind(
  cbind(object = 1, part = 1, outline, hole = 0),
...
clinic <- terra::vect(clinic, "polygons")
terra::writeVector(
  clinic, "data-raw/building/clinic-vector.gpkg",
  overwrite = TRUE,
  filetype = "GPKG"
)
