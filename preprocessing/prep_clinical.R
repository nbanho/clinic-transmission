...
df_pat_time_masi <- read.csv("data-raw/clinical/patient-time.csv") %>%
  ...
  mutate(time = parse_date_time(paste(date, time), '%m/%d/%Y %I:%M:%S %p', tz = "CET"),
         date = as.Date(time)) %>%
  filter(!is.na(time))
...
df <- df_pat_time_masi %>%
  ...
  mutate(diff_treat_start_1 = as.numeric(date - tb_treat_start),
...
saveRDS(df, "data-clean/clinical/clinical-data.rds")
