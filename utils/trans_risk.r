...
rundiag <- function(n, empirical = T, counts = c(6, 12, 8, 2, 1), prev_est = "RSA") {
  if (empirical) {
...
#' by McCreesh who references Dharmadhikari for the reduction with face masks worn by patients with
#' multi-drug resistant TB.
#'
#' @references
#' - McCreesh: MCreesh et al. (2021) BMJ Global Health (doi.org/10.1136/bmjgh-2021-007124)
#' - Dharmadhikari: Dharmadhikari et al. (2012) in Am J Respir Crit Care Med (doi.org/10.1164/rccm.201107-1190OC)
#'

fit_beta <- function(mean, lower, upper) {
  # Calculate the sample variance
  variance <- ((upper - lower) / (qnorm(0.975) * 2))^2
...
rrmask_par <- fit_beta(.75, .56, .85)

rrmask <- function(n) {
  rbeta(n, rrmask_par[1], rrmask_par[2])
}
...
#'
#' @details
#' See Summary Table 1 from Adam's paper.
...
IR <- function(activity, sex) {
  IR <- matrix(
    ...
    ncol = 2, byrow = T,
    dimnames = list(
      c("resting", "sitting", "standing", "walking"),
      c("male", "female")
    )
  )
  IR <- IR / 1000 * 60
...
#' - Adams: Adams (1993) [Report] Measurement of Breathing Rate and Volume in Routinely Performed Daily Activities.
#' - Stadnytskyi: Stadnytsk yi et al. (2020) in PNAS (doi.org/10.1073/pnas.2006874117)
...
estimate_aer <- function(data, G = 0.004 * 60, dt = 1 / 60, aer_range = c(0.01, 10, 1000), Cr_range = c(300, 450, 600)) {
  res <- optim(
    par = c(aer_range[2], Cr_range[2]), fn = min_rss, data = data, G = G, dt = dt,
